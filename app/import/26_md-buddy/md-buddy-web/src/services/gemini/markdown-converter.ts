...
重要な指示：
- 提供された音声またはテキストの実際の内容のみを使用してください
- 架空の内容や例文を生成しないでください
- 実際に話された内容や書かれた内容に忠実に基づいてください
- ダミーデータやサンプルデータを作成しないでください
- 内容が不明な場合は推測せず、その旨を明記してください`;
...
重要なポイント：
- 実際の参加者の発言を要約し、重要な決定事項を明確にしてください
- 実際に言及されたアクションアイテムと担当者、期限を特定してください
- 実際の議論の流れが分かりやすい構造にしてください`;
...
重要なポイント：
- 実際の内容に基づいて適切な見出し構造（H1, H2, H3）を使用してください
- 実際に述べられた重要なポイントは太字や箇条書きで強調してください
- 実際の内容を読者にとって価値のある形に整理してください`;
...
重要なポイント：
- 実際に述べられた主要なポイントを漏れなく含めてください
- 実際の内容を簡潔で理解しやすい文章にしてください
- 実際の内容の重要度に応じて情報を整理してください`;
...
重要なポイント：
- 実際の内容に基づいて読者の興味を引く導入を書いてください
- 実際に提供された情報から実用的で価値のある内容を抽出してください
- 実際の内容を親しみやすく読みやすい文体で表現してください`;
...
重要なポイント：
- 実際に説明された手順や内容を明確で具体的にしてください
- 実際に言及されたコードやデータがある場合のみコードブロックやテーブルを使用してください
- 実際の内容の技術的な正確性を保ってください`;

      case ConversionType.CUSTOM:
      default:
        return `提供された音声またはテキストから、適切に構造化されたMarkdown文書を作成してください。
${baseInstructions}

実際の内容に応じて最適な形式を選択し、読みやすく有用な文書にしてください。`;
    }
  }
